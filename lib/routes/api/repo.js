 ...
 *    curl -X DELETE http://localhost/api/strider/strider-cd/cache
 */
...
function clearProjectCache(project, cb) {
  ...
  runners.forEach(function (rid) {
    ...
    console.log(rid, common.extensions.runner, project);
 ...
 *
 * @apiExample {curl} CURL Example:
 *    curl -X DELETE http://localhost/api/strider/strider-cd
 */
exports.delete_project = function(req, res) {
  async.parallel([
    ...
    function (next) {
      clearProjectCache(req.project, function (error, result) {
        next(error);
      });
    },
    function (next) {
...
var error = function(ctx, err_msg, res){
  ...
  return res.status(400).end(JSON.stringify(r, null, '\t'));
}
