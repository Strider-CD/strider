...
function clearProjectCache(project, cb) {
  ...
  project.environments.forEach(function (environment) {
    var branchSources = environment.sources.filter((item) => item.type === 'branch');
    var runnerId = environment.runner.id;

    if (!branchSources.length || runners.indexOf(runnerId) !== -1) {
      return;
    }
      ...
      environments: [
        {
          ...
          phases: {
            deploy: {
              runOnFailedTests: false,
              runForPullRequests: false
            }
          },
          sources: [
            {
