...
exports.createProject = function(req, res, next) {
  ...
  if (
    req.body.prefetch_config === 'false' ||
  ...
  function projectResult(err, project) {
    if (project) {
      debug(
        `User ${req.user.email} tried to create project for repo ${name}, but it already exists`
      );

      return error(409, 'project already exists');
    }

    return ssh.generateKeyPair(
      `${name}-${req.user.email}`,
  ...
  function createProjectWithKey(err, privkey, pubkey) {
    ...
    debug(
      `Setting up repository "${project.name}" with provider "${provider.id}"...`
    );
    plugin.setupRepo(
      req.user.account(provider).config,
      provider.config,
      project,
      function(err, config) {
        if (err) {
          debug(
            `Setting up repository "${project.name}" failed!`,
            err.status,
            err.message
          );
  ...
  async.parallel(
    [
      function(next) {
        ...
        plugin.teardownRepo(
          req.project.creator.account(provider).config,
          provider.config,
          req.project,
          function(err) {
            if (err)
              debug(
                'Error while tearing down repo',
                req.project.name,
                provider.id,
                err
              );
    ...
    function(err) {
      if (err) {
        debug(
          'repo.delete_index() - Error deleting repo config for url %s by user %s: %s',
          req.project.name,
          req.user.email,
          err
        );
