...
function mergeConfigs(branch, striderJson) {
  ...
  const allSchemas = _.keys(common.extensions).reduce(
    (allSchemas, extensionType) => {
      _.keys(common.extensions[extensionType]).reduce(
        (allSchemas, pluginId) => {
          if (extensionType === 'runner') return allSchemas;
          allSchemas[pluginId] = defaultSchema(
            common.extensions[extensionType][pluginId]
          );
          return allSchemas;
        },
        allSchemas
