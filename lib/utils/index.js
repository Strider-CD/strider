...
function findEnvironment(environments, name) {
  ...
  environments.some(function (environment) {
    if (environment.name) {
      var regEx = new RegExp(`^${environment.name.replace(/\*/g, '.*')}$`);
      if (regEx.test(name)) {
  ...
  return (function discreteEnvironmentFn(name, environment, environments) {
    ...
    if (environment.name !== name) {
      ...
      var discreteBranch = _.find(environments, {name: name});
      if (discreteBranch) {
        environment = discreteBranch;
    ...
    return environment;
  }(name, foundEnvironment, environments));
}
