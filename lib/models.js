...
var RepoConfig = new Schema({
  ...
  , url: String
  , friendly_url: String
  // not sure how to do this in Mongoose ...
  , auth: {
    , type: 'https'
    , username: String // [oauth key here]
    , password: String // [empty if we're using GH oauth]
  } || {
    , type: 'ssh'
    , pubkey: String
    , privkey: String
  ...
  , secret: {type: String }
  // things outside of config are only modifyable by admins
  // things in config can also be set by a .strider.json file in the repo
  , config : {
...
/**
 * findCollaborators(list_of_collaborators, next(null, whitelist))
 *
 * Whitelist contains emails & access levels for each collaborator.
 */
UserSchema.statics.findCollaborators = function (collaborators, next) {
  var ids = collaborators.map(function (c) { return c.user_id })
  User.find({_id: {$in: ids}}, {email: 1}, function (err, results) {
    ...
    collaborators.forEach(function (collab) {
      var email = map[collab.user_id]
      if (!email) { // user has been deleted...
        console.warn('Skipping collaborator that was deleted...', collab.user_id)
        return
      }
      whitelist.push({
        owner: false,
