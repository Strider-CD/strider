...
var JobSchema = new Schema({
  ...
  archived: Date,
  test_exticode: Number,
  deploy_exticode: Number,
  errored: {type: Boolean, default: false}

  // XXX what are these "tasks" ???
...
UserSchema.methods.get_repo_config = function(case_insensitive_url, cb) {
  ...
  console.debug("user.get_repo_config() - no repos found matching %s in %j, querying DB",
    ...
    // Look for user object with the project url configured AND
    // current user id is in collaborators list OR repo is public.
