...
UserSchema.static('create', function (email, password, accountLevel, cb) {
  if (!email || !password) {
    return process.nextTick(function () {
      cb('Invalid arguments for User.create(..)');
    });
  }
...
UserSchema.static('registerWithInvite', function(inviteCode, email, password, cb) {
  ...
  InviteCode.findOne({
    ...
    user.save(function (error, user) {
      if (error) {
        return cb('Error Creating User:' + error);
      }
