...
async function striderJson(provider, project, environment) {
  let getFile = promisify(provider.getFile);
  let contents;
  let data;
  let ref = {
    branch: environment.manualTriggerBranch
  };

  try {
    if (!provider.hosted) {
      contents = await getFile('strider.json', ref, project.provider.config, project);
    }

    let account = project.creator.account(project.provider.id, project.provider.account);

    contents = await getFile('strider.json', ref, account.config, project.provider.config, project);
  } catch(err) {
    throw err;
  }
  ...
  try {
    ...
    debug('Strider config is invalid JSON for', project, environment);
    debug(contents);
  }
