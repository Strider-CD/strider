...
exports.init = function() {
  ...
  common.emitter.on('job.new', function(job) {
    ...
    User.collaborators(name, 0, function (err, users) {
      ...
      websockets.send(ui_users[name], 'job.new', njob, 'yours')
      if (job.project.public) {
        websockets.sendPublic(ui_users[name], 'job.new', njob, 'public')
      }
  ...
  function fire(project, event, args) {
    if (ui_users[project]) {
      websockets.send(ui_users[project], event, args, 'yours')
    }
    if (public_projects[project]) {
      websockets.sendPublic(ui_users[project] || [], event, args, 'public')
    }
  }
  ...
  common.emitter.on('job.done', function (data) {
    Job.findById(data.id, function (err, job) {
      ...
      Project.findOne({name: job.project}).lean().exec(function (err, project) {
        ...
        var njob = utils.sanitizeJob(job)
        job.status = njob.status
        njob.project = job.project
        websockets.send(ui_users[project.name], 'job.done', job, 'yours')
        if (njob.project.public) {
          websockets.sendPublic(ui_users[project.name], 'job.done', njob, 'public')
        }
        // TODO: also send to system admins
