...
function striderJson(provider, project, ref, done) {
  function finished(err, contents) {
    if (err || !contents) return done(err, null)
    var data = {}
    try {
...
function prepareJob(emitter, job) {
  Project.findOne({ name: job.project }).populate('creator').exec(function (err, project) {
    if (err || !project) {
      return console.error('job.prepare - failed to get project', job.project, err);
    }
    ...
    Job.create(job, function (err, mjob) {
      if (err) {
        return console.error('job.prepare - failed to save job', job, err);
      }
      ...
      striderJson(provider, project, job.ref, function (err, config) {
        if (err) {
          return console.error('[job.prepare] project config error', project.name, err);
        }
