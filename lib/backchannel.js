...
function striderJson(provider, project, ref, done) {
  ...
  const account = project.creator.account(
    project.provider.id,
...
function prepareJob(emitter, job) {
  Project.findOne({ name: job.project })
    ...
    .exec(function (err, project) {
      ...
      if (!provider) {
        return debug(
          'job.prepare - provider not found for project',
          job.project,
          project.provider.id
        );
      }
        ...
        striderJson(provider, project, job.ref, function (err, config) {
          if (err) {
            if (err.status === 403 || err.statusCode === 403) {
              debug(
                'job.prepare - access to strider.json is forbidden, skipping config merge'
              );
              config = {};
              jjob.fromStriderJson = false;
            } else if (err.status === 404 || err.statusCode === 404) {
              debug(
                'job.prepare - strider.json not found, skipping config merge'
              );
              config = {};
              jjob.fromStriderJson = false;
            } else {
              debug(
                "job.prepare - error opening/processing project's `strider.json` file: ",
                err
              );
              config = {};
              jjob.fromStriderJson = false;
          ...
          if (!common.extensions.runner[config.runner.id]) {
            debug(
              `Error: A job was registered for a runner that doesn't exist, i.e. "${config.runner.id}". This job will never run!`
            );
          }
      ...
      this.ws.sendEach(this.users[project], function (user) {
        return [
          ...
          args.map(function (job) {
            ...
            job.project.access_level = User.projectAccessLevel(
              user,
              job.project
            );
            return job;
      ...
      (err, users) => {
        ...
        if (Array.isArray(waiting)) {
          waiting.forEach((item) => {
            this.send.apply(this, [name].concat(item));
          });
        }
  ...
  onUpdate: function (project, event, args) {
    ...
    if (event === 'job.status.started') {
      Job.findById(args[0], function (err, job) {
        if (err)
          return debug(
            '[backchannel][job.status.started] error getting job',
            args[0],
            err
          );
        if (!job)
          return debug(
            '[backchannel][job.status.started] job not found',
            args[0]
          );
        job.started = args[1];
        job.save();
