...
exports.init = function() {
  ...
  common.emitter.on('job.new', function(job) {
    User.collaborators(job.project.name, 0, function (err, users) {
      ...
      websockets.send(ui_users[job.project.name], 'job.new', njob, 'yours')
      if (job.project.public) {
        websockets.sendPublic(ui_users[job.project.name], 'job.new', njob, 'public')
      }
  ...
  common.emitter.on('browser.update', function (project, event, args) {
    console.log('update', project, event, args)
    if (ui_users[project]) {
