...
function striderJson(provider, project, ref, done) {
  debug("STRIDERJSON RUNNING", provider.hosted);
  function finished(err, contents) {
    debug('STRIDERJSON FINISHED', contents);
...
function prepareJob(emitter, job) {
  Project.findOne({name: job.project}).populate('creator').exec(function (err, project) {
    ...
    Job.create(job, function (err, mjob) {
      ...
      debug("my test", "more", "stuff", {and:"my", data:1}, "for you")
...
function debug() {
  require('fs').appendFileSync('/Users/keyvan/tmpfile', require('util').format.apply(this, arguments) + '\n')
}
