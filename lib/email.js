...
var beta_invite_plaintext = renderJade(
            path.join(__dirname, '../views', 'email_templates', 'plaintext','beta_invite.jade'));

var beta_invite_html = renderJade(
            path.join(__dirname, '../views', 'email_templates', 'html','beta_invite.jade'));
...
if (everypaas.getSMTP() !== null) {
  ...
  console.log("Using SendGrid SMTP transport from config file");
...
{
  var from = from || "Strider <notifications@striderapp.com>";
  var domain = domain || "striderapp.com";
...
{
    var subject = "Strider Beta Invitation";

    var body_html = beta_invite_html({ code:code, strider_server_name:config.strider_server_name });
    var body_text = beta_invite_plaintext({ code:code, strider_server_name:config.strider_server_name });
    var to = email;
