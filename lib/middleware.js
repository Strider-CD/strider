...
function projectProvider(req, res, next) {
  ...
  if (project.provider.account) {
    ...
    req.accountConfig = function (config, next) {
      if (arguments.length === 0) return account.config
...
function projectPlugin(req, res, next) {
  ...
  req.userConfig = function (config, next) {
    ...
    if (!schema) return next(new Error('Plugin ' + pluginid + " doesn't define any user config"))
...
function anonProject(req, res, next) {
  ...
  Project.findOne({name: name}).populate('creator').exec(function (err, project) {
    if (err) {
      res.status(500)
    ...
    if (req.user && req.user.projects) {
      p = _.find(req.user.projects, function(p) {
        return p.name === name
      })
    }
