...
var init = exports.init = function (config) {
  ...
  app.configure('development', function(){
    ...
    app.get('/test/:name', function (req, res) {
      var data
        , filename = path.join(__dirname, '../views/test/' + req.params.name + '.json')
      if (require.cache[filename]) delete require.cache[filename]
      try {
        data = require(filename)
      } catch (e) {
        console.warn('Failed to load data for ', req.params.name, e)
        data = {}
      }
      data.log = console.log.bind(console)
      res.render('test/' + req.params.name + '.html', data)
    })
  });
