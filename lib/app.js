...
var init = exports.init = function (config) {
  ...
  swig.init({
    ...
    , extensions: {plugin: require('./pluginTemplates')} 
  });
  ...
  app.get('/add_repo', middleware.require_auth_browser, function(req, res) { res.render('add_repo.html'); });

  /* Requires at least read-only access to the repository in the path */
  app.get('/:org/:repo/latest_build', middleware.require_resource_read, routes_jobs.latest_build);
  app.get('/:org/:repo/', middleware.require_resource_read, routes_jobs.latest_build);
  app.get('/:org/:repo/job/:job_id', middleware.require_resource_read, routes_jobs.job);
  ...
  app.get('/manual_setup', middleware.require_auth, function(req,res) {res.render('manual_setup.html');});
