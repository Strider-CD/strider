...
var init = exports.init = function (config) {
  ...
  // auto_reconnect seems broken in mongoose 3.6.9, so we just exit
  // in hopes an application restart will sort it out.
  // see 
  mongoose.connection.on('error', function(e) {
    console.error("MongoDB connection error: %s", e);
    process.exit(1);
  });
  session_store = new mongoStore({url: mongodbUrl, auto_reconnect:true});
