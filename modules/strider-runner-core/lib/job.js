...
function Job(job, provider, plugins, config) {
  ...
  debug(`Creating new job "${jobId}" for project "${job.project.display_name}" using provider "${provider.id}".`);
...
Job.prototype = {
  ...
  runPhase: function (next) {
    ...
    this.plugins.forEach(function (plugin) {
      ...
      if ('string' === typeof plugin.path) {
        ...
        path = path.concat('function' === typeof plugin.path ? plugin.path(self.phase) : plugin.path);
      }

      tasks.push(function (next) {
        utils.runPlugin(self.phase, plugin, self.pluginContext(plugin.id, env, path), next);
      });
    });
  ...
  phaseDone: function (next, started, err, actuallyRan) {
    ...
    if (['deploy', 'test'].indexOf(this.phase) !== -1 && utils.sum(actuallyRan) === 0) {
